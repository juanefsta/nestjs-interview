...
export class QueueProcessor {
  ...
  async handleSync(job: Job) {
    ...
    try {
      ...
      this.logger.log(
        `✅ Job ${job.id} completed successfully. Successfully processed ${operation} for item ${data.id}`,
      );
    } catch (error) {
      this.logger.error(
        `❌ Job ${job.id} failed for operation -${operation}- and item ${data.id}. Attempt ${job.attemptsMade + 1}. Error: ${error.message}`,
      );
      throw error;
    }
  }
