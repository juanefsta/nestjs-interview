...
describe('QueueProcessor', () => {
  ...
  it('should call createTodoList for create operation', async () => {
    const job = {
      ...
      attemptsMade: 0,
      opts: { attempts: 3 },
      data: { operation: 'createTodoList', data: { id: 1 } },
    } as Job;
  ...
  it('should call updateTodoList for update operation', async () => {
    const job = {
      ...
      attemptsMade: 1,
      opts: { attempts: 3 },
      data: { operation: 'updateTodoList', data: { id: 2 } },
    } as Job;
    await processor.handleSync(job);
  ...
  it('should call deleteTodoList for delete operation', async () => {
    const job = {
      ...
      attemptsMade: 2,
      opts: { attempts: 3 },
      data: { operation: 'deleteTodoList', data: { id: 3 } },
    } as Job;
    await processor.handleSync(job);
  ...
  it('should log and throw error for unknown operation', async () => {
    const job = {
      ...
      attemptsMade: 0,
      opts: { attempts: 3 },
      data: { operation: 'unknown', data: {} },
    } as Job;
    await expect(processor.handleSync(job)).rejects.toThrow(
      'Unknown operation: unknown',
    );
    expect(processor.handleSync(job)).rejects.toThrow(
      'Unknown operation: unknown',
    );
  });
});
...
describe('QueueService', () => {
  ...
  it('should add create job to queue', async () => {
    ...
    expect(queue.add).toHaveBeenCalledWith(
      'synchronize',
      { operation: 'createTodoList', data: { id: 1 } },
      expect.any(Object),
    );
  });
  ...
  it('should add update job to queue', async () => {
    ...
    expect(queue.add).toHaveBeenCalledWith(
      'synchronize',
      { operation: 'updateTodoList', data: { id: 2 } },
      expect.any(Object),
    );
  });
  ...
  it('should add delete job to queue', async () => {
    ...
    expect(queue.add).toHaveBeenCalledWith(
      'synchronize',
      { operation: 'deleteTodoList', data: { id: 3 } },
      expect.any(Object),
    );
  });
});
