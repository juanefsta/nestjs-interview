...
> {
  constructor(
    protected readonly queueService: QueueService<TodoItem>,
    @Inject(forwardRef(() => TodoListsService))
    protected readonly todoListService: TodoListsService,
  ) {
  ...
  create(dto: CreateTodoItemDto, syncDisabled?: boolean): TodoItem {
    ...
    const updatedList = this.todoListService.update(
      createdItem.listId,
      { updated_at: new Date() },
      true,
    );
  ...
  findAllByKeyId(listId: number): TodoItem[] {
    return this.items.filter(
      (item: TodoItem) => Number(item.listId) === Number(listId),
    );
  }
}
